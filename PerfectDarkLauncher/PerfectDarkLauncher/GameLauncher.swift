...
class GameLauncher {
    ...
    func launchGame(settings: GameSettings) -> Result<Void, LaunchError> {
        ...
        // Determine working directory (where pd.ini will be written)
        let configDirectory = getConfigDirectory(settings: settings)
        
        // Create/update config file in the config directory
        if case .failure(let error) = writeConfigFile(settings: settings, directory: configDirectory) {
            return .failure(error)
        }
        
        // Setup ROM file (copy or symlink if needed)
        if case .failure(let error) = setupRomFile(settings: settings, directory: configDirectory) {
            return .failure(error)
        }
        ...
        // Save directory (where pd.ini is) - point to where we wrote the config
        arguments.append("--savedir")
        arguments.append(configDirectory)
        ...
        do {
            ...
            // Set working directory to ROM directory for data file access
            let romDirectory = (settings.romPath as NSString).deletingLastPathComponent
            process.currentDirectoryURL = URL(fileURLWithPath: romDirectory)
            
            // Set up environment
            var environment = ProcessInfo.processInfo.environment
            environment["DYLD_LIBRARY_PATH"] = romDirectory
    ...
    private func getConfigDirectory(settings: GameSettings) -> String {
        // Write pd.ini next to the ROM file - this keeps all user data together
        // and makes it easy to manage multiple ROM configurations
        return (settings.romPath as NSString).deletingLastPathComponent
    }
    ...
    private func writeConfigFile(settings: GameSettings, directory: String) -> Result<Void, LaunchError> {
        ...
        // Write pd.ini next to the ROM file - this keeps configuration with the ROM
        // and allows for different configs per ROM if the user has multiple ROMs
        // Key names must match those registered in the game's video.c and main.c
        let config = """
        ; Perfect Dark PC Port Configuration
        ; Generated by Carrington Launcher
