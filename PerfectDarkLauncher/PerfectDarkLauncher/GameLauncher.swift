...
class GameLauncher {
    ...
    private func writeConfigFile(settings: GameSettings, directory: String) -> Result<Void, LaunchError> {
        ...
        CenterWindow=\(settings.centerWindow ? 1 : 0)
        ...
        DisplayFPSInterval=\(String(format: "%.2f", settings.displayFPSInterval))
        ...
        TickRateDivisor=\(settings.tickRateDivisor)
        ExtraSleep=\(settings.extraSleep ? 1 : 0)
        ...
        CrosshairHealth=\(settings.crosshairHealth)
        CrouchMode=\(settings.crouchMode.rawValue)
        ExtendedControls=\(settings.extendedControls ? 1 : 0)
        RadialMenuSpeed=\(String(format: "%.2f", settings.radialMenuSpeed))
        UseKeyReloads=\(settings.useKeyReloads ? 1 : 0)
        
        [Input]
        MouseEnabled=\(settings.mouseEnabled ? 1 : 0)
        MouseSpeedX=\(String(format: "%.2f", settings.mouseSpeedX))
        MouseSpeedY=\(String(format: "%.2f", settings.mouseSpeedY))
        FakeGamepads=\(settings.fakeGamepads)
